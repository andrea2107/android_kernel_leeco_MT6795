...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		 if(!strcmp(fwu->product_id_string, "S332U") ||!strcmp(fwu->product_id_string, "X3-03-01"))
        {
...
static int synaptics_fw_updater(const unsigned char *fw_data)
{
	int retval;
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (boot_mode == META_BOOT) {
		dev_dbg(&rmi4_data->i2c_client->dev,
				"%s: Don't update fw in META_BOOT\n",
				__func__);
		return 0;
	}
...
{
	synaptics_rmi4_new_function_l(&fwu_module, true);
...
{
	synaptics_rmi4_new_function_l(&fwu_module, false);
